...
def data_get():
    city=city_name.get()
    data = requests.get("https://api.openweathermap.org/data/2.5/weather?q="+city+"&appid=4eea9b9f61abbf3743a24ac8d5886c73").json()
    w_label1.config(text=data["weather"][0]["main"])
    wb_label1.config(text=data["weather"][0]["description"])
    temp_label1.config(text=str(data["main"]["temp"]-273.15))
    min_label1.config(text=str(data["main"]["temp"]-278.15))
    max_label1.config(text=str(data["main"]["temp"]-269.15))
...
name_label=Label(win,text="WSCUBE Tech",
...
com=ttk.Combobox(win,text="WSCUBE Tech",values=list_name,
...
min_label=Label(win,text="Minimum Temperature",
                 font=("Time New Roman",15))
...
max_label=Label(win,text="Maximum Temperature",
                 font=("Time New Roman",15))
